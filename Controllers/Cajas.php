<?php
    class Cajas extends Controller{
        ...
        public function listar(){
            ...
            for ($i=0; $i < count($data) ; $i++) {
                if($data[$i]['estado'] == 1){
                    ...
                    $data[$i]['acciones'] = '<div>
                    <button class="btn btn-success" type="button" onclick="btnReingresarCaja('.$data[$i]['id'].');" ><i class="fas fa-reply"></i></button>
                    </div>';
                }
            }
        ...
        public function registrar(){
            ...
            if(empty($caja)){
                $msg = "Nombre de la caja es necesario";
            }else{
                if($id == ""){
